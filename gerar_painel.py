...
try:
    ...
    colunas_metricas = ['quantidade', 'cotas_ativas_em_dia', 'cotas_ativas_contempladas_acum', 'cotas_ativas_credito_pendente', 'cotas_ativas_quitadas', 'cotas_ativas_nao_contempladas_inadimplentes', 'cotas_ativas_contempladas_inadimplentes', 'cotas_excluidas_a_comercializar', 'cotas_ativas_contempladas_mes', 'cotas_ativas_total']
    ...
    fig_funil = go.Figure(go.Funnel(
        ...
        x=[df['quantidade'].sum(), df['cotas_ativas_em_dia'].sum(), df['cotas_ativas_contempladas_acum'].sum(), df['cotas_ativas_credito_pendente'].sum(), df['cotas_ativas_quitadas'].sum()],
    ...
    # Imprime o HTML gerado para ser lido pelo PHP
    ...
    <body class="p-4">
        <div class="container">
            <h2>Painel da Carteira (Processado via Python/PHP)</h2>
            <div>{html_funil}</div>
        </div>
    </body>
    </html>""")
    ...
    print(f"<h3>Erro no processamento Python: {e}</h3>")
